...
Those roles must stay separate in production/beta. Do not run the Render worker and Railway worker against the same production queue at the same time unless you intentionally want multiple workers and have tested lease/reclaim behavior.
...
`AUTH_SECRET`, `DATABASE_URL`, `DIRECT_URL`, `REDIS_URL`, `OPENAI_API_KEY`, Supabase env, and `EVIDENCE_SIGNING_SECRET` must match the API service.
...
10. Stop the Render worker before pointing production traffic at Railway worker.
11. Update Vercel `NEXT_PUBLIC_API_URL` to the Railway API URL.
12. Redeploy Vercel and verify browser network calls go to Railway, not Render.
13. Keep Render services paused for rollback until Railway has survived 24-48 hours of beta traffic.

## Rollback

If Railway fails before Vercel cutover, keep Vercel pointed at the old API and fix Railway in isolation.

If Railway fails after Vercel cutover:

1. Pause Railway worker to stop new queue claims.
2. Restore Vercel `NEXT_PUBLIC_API_URL` to the last known-good API URL.
3. Resume the old worker only after confirming it points to the intended Redis queue.
