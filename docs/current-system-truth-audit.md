...
## PR G — Production canary plan + integration hardening milestone report (Phase 13, NAMED STOP)

**Status:** PLAN + safe local dry-run delivered on `chore/phase13-production-canary-plan` (off main `d7f1a18` = PR F merged). The live production canary is a **NAMED STOP** — it requires production credentials not available in this session. Docs-only PR (no source touched).

**What this PR delivers (the parts that need no credentials):**
1. `docs/production-canary-plan.md` — the executable 6-stage live canary runbook: Stage 1 audit-chain + merge atomicity against a staging schema on managed Postgres (non-destructive); Stage 2 Shield MCP observational canary (`SHIELD_MCP_CANARY=1`, records only, does not gate); Stage 3 Brain MCP canary (`BRAIN_MCP_SERVER=1`); Stage 4 `executeApprovedSpec` live E2E (one REVIEWER-approved spec); Stage 5 HyperAgent self-learning 1%-traffic 7-day human-promote-only canary (the "never enable self-learning globally without a canary" constraint); Stage 6 persisted benchmark. Each stage has a kill switch + observation window. The canary-flag inventory is verified against source (`hyperAgentEnabled`, `JAK_ADK_MODE`, `BRAIN_MCP_SERVER`, `SHIELD_MCP_CANARY`, `BENCHMARKS_PERSISTED` — all default-off).
2. The safe non-destructive local dry-run (§2 of the plan) — exercises every canary flag against a local pgvector testcontainer with a deterministic stub `runPlan`, no prod creds. Verified green in this session (audit-chain concurrency, merge atomicity, shield-mcp-live-audit, brain-mcp-server suites).
3. `docs/mandate-completion-report.md` — the mandated 24-item integration hardening milestone report (renamed from "final completion report" — see its scope note) covering the whole 14-phase / 7-PR (A–G) mandate: per-PR outcomes, gates honored, tenant-safety/transactional/observability/evidence proofs, honest "NOT production-proven" boundaries, test totals, migrations, backup-branch disposition, the STOP + owner input requested, known gaps, and the final verdict.

**STOP (the named stop condition):** the live canary (plan §3) requires five production secrets (managed `DATABASE_URL`, `OPENAI_API_KEY` or `GEMINI_API_KEY`, `EVIDENCE_SIGNING_SECRET`, `SHIELD_SIGNING_KEY`, `SHIELD_VERIFICATION_KEY`) + a staging Cloud Run revision + a non-prod schema + explicit owner go/no-go for the self-learning canary + an owner-generated Ed25519 Shield keypair. None are in this session; none were faked. Owner input requested (plan §5 / completion report item 22).

**Honest limitations:** this PR is plan + report, NOT a run canary. No production evidence was produced. The "NOT production-proven" items (measured learning impact in prod, live `executeApprovedSpec` LLM E2E, Brain/Shield canaries against live traffic, audit-chain/merge against managed Postgres) remain env-blocked exactly as before — PR G makes them *runnable* by an owner, not *run*.
