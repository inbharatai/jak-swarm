...
- **HyperAgent self-healing + self-learning layer** (Phases 0-15, merged to `main`): deterministic 20-class failure classifier → counterfactual diagnosis → symbolic replanner + validator → autonomy-gated apply; information-theoretic learning gate; versioned-config lifecycle (shadow/canary/promote/rollback); R5 code self-repair (isolated branch + draft PR only); honest Control Centre (9 views). A subsequent **live-wiring engagement** (Phases 1-7) then wired the self-learning half into the live LangGraph execution path (OFF-gated, non-regressing): live learning persist service (dedup/merge/promote/scope) → learning node → enablement plumbing + planner recall + bandit selection → measured learning impact proven at the live-graph integration level → R2 `CORRECT_OUTPUT` typed correction → `executeApprovedSpec` closed loop (REVIEWER-gated) → `AuditLog` HMAC per-tenant row-chain (migration 117). Integration tests prove real plan repair, measured learning promotion at the live-graph level, Shield fail-closed, rollback, and AuditLog chain verify + tamper detection end-to-end. **Honest scope:** both halves are wired into the live LangGraph graph (OFF-gated, `hyperAgentEnabled` defaults false); retry accounting is unified on `taskRetryCount` + a single shared `MAX_TASK_RETRIES=2` (a 16-bug hardening sweep also landed host-JS production-disable, a clearable graph `error` channel, Shield subject-substitution defense, a DP-noise scale fix, cyclic-spec rejection, code-repair path normalization, a node-timeout timer-leak fix, and an unbypassable risk-classifier security floor). **Deferred:** the `ShieldMcpClient` live-action instantiation + external MCP transport. **Env-blocked, not fake-passed:** *measured learning impact in production* (12-step E2E + Cloud Run deploy gate — integration-graph-proven, not production-proven), live-launch of `executeApprovedSpec` in prod, `AuditLog` chain live-Postgres round-trip + its fetch-latest-then-write TOCTOU under concurrency.
...
**Phase 11B:** Wire JAK Shield MCP for high-risk action validation. The `ShieldMcpClient` (local embedded mode: signs + verifies decisions with a tenant key pair, fail-closed when unavailable/unverified) and the `AgentGovernanceOverlay` autonomy/governance core are **built** (HyperAgent Phases 7-8). `AuditLog` now has a per-tenant HMAC row-chain (migration 117, fail-open-to-auditable when `EVIDENCE_SIGNING_SECRET` is unset). What remains is the **MCP transport** call from JAK Swarm to the external JAK Shield service, instantiating the `ShieldMcpClient` in the live action path, and routing Shield decisions through the existing audit chain.
