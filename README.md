...
|---|---|---|
| Evidence ingestion | Source-labeled artifacts from docs, tickets, code, meetings, customer calls, Slack, Notion, GitHub, Gmail, Google Drive, or manual upload. Tenant-scoped, body-hashed. | GitHub, Gmail, Google Drive auto-sync (5-min). Other 19/22 connectors ingest on-demand/manual. |
...
| Retrieval | Multi-signal lexical + graph retrieval: alias + stable-identifier ILIKE + `ts_rank` FTS + 1-hop graph expansion. **No vector / embedding / pgvector component.** | Shipped, honest. Not "hybrid retrieval" — there is no vector half. |
