...
| Safety / security layer | JAK Shield-style local policy controls, RBAC, approval gates, audit logging (with **per-tenant HMAC row-chain tamper-evidence**, `EVIDENCE_SIGNING_SECRET`-gated, fail-open-to-auditable when unset), PII redaction, HMAC-ready security path + a **`ShieldMcpClient`** that signs/verifies high-risk decisions (local embedded mode, unit-tested) and an **Agent Governance Overlay** (autonomy policy L0-L5 + human-only NEVER set, governance gate). The `ShieldMcpClient` is **not yet instantiated in the live action path** (deferred) and the external JAK Shield MCP *transport* wiring remains a roadmap item. Today the enforced gateway is the embedded local one | ✅ Verified |
...
> **Bounded retry + genuine re-plan loop**: If the Verifier rejects output, the system re-runs the same task against the same worker up to a bounded retry budget (`RepairService` classifies the failure and refuses to auto-retry destructive / permission / approval-timeout / unknown classes). Beyond retry, the **HyperAgent self-healing layer** (merged to `main`, Phases 0-15) adds a genuine *re-plan* loop that is **wired into the live LangGraph graph** (OFF-gated so default workflows are byte-for-byte unchanged): a deterministic 20-class failure classifier → counterfactual fault isolation (agent/tool/model) → a **symbolic replanner** that proposes agent/tool/task-order/dependency/scope changes and a **deterministic validator** that rejects cyclic / unpermitted / unsafe plans (Innovation #3 — an LLM may propose, only the symbolic layer may apply) → autonomy-gated apply that preserves completed external actions + signed approvals + receipts. Integration tests prove real plan repair (REPLACE_AGENT, SPLIT_TASK), learning promotion via measured mutual information, Shield fail-closed, and versioned-config rollback end-to-end — *by composing the pure cores directly*. **Honest scope:** the *repair* half (diagnosis → replanner → selective re-execution) AND the *self-learning* half (outcome evaluator + acceptance checker, learning extraction / persist / recall, mutual-information gate, meta-optimiser/bandit selection, R2 `CORRECT_OUTPUT` typed correction) are both **wired into the live LangGraph graph** — OFF-gated by `hyperAgentActive(state)` so default workflows are byte-for-byte unchanged unless a tenant opts in. The `executeApprovedSpec` closed loop (spec → materialise → run → harvest → acceptance verdict → resolved drift) and the `AuditLog` HMAC per-tenant row-chain (tamper-evident, fail-open-to-auditable when `EVIDENCE_SIGNING_SECRET` is unset) are now implemented and integration-graph-proven; the `ShieldMcpClient` is the remaining pure-core (signs/verifies/fail-closed at the unit level, not yet instantiated in the live action path — deferred). Retry accounting was unified onto `taskRetryCount` + a single shared `MAX_TASK_RETRIES=2` (verifier + edge now read the same field/constant, so they can no longer disagree), and a 16-bug hardening sweep landed further safety fixes (host-JS `code_execute` production-disabled because Node `vm` is not a security boundary; the graph-state `error` channel made clearable so recovered runs are no longer mis-reported as failed; Shield subject-substitution defense; DP-noise scale corrected to the reported sensitivity; cyclic-spec rejection; code-repair path normalization; node-timeout timer-leak fix; risk-classifier security floor unbypassable) — see the HyperAgent section below for the full wired-vs-pure-core breakdown. Destructive actions are never auto-retried. **What is still env-blocked, not fake-passed:** the live 12-step E2E + Cloud Run deploy gate that would prove *measured learning impact in production* (integration-graph-proven at the live LangGraph level, NOT production-proven), the live-launch of `executeApprovedSpec` in prod (the REVIEWER-gated API route + service exist, but the live LangGraph+LLM E2E is env-blocked), the `AuditLog` chain's live-Postgres round-trip + its fetch-latest-then-write TOCTOU under concurrency, and the external JAK Shield MCP network transport (`ShieldMcpClient` wiring deferred). See `docs/hyperagent-current-state-audit.md` (Phase 0 baseline).
...
- 🟡 **Deferred — pure core, NOT yet instantiated in the live action path:** the `ShieldMcpClient` (signs/verifies/fail-closed at the unit level; local-embedded + external MCP transport wiring is deferred; when `JAK_SHIELD_MCP_URL` is unset the behavior is identical to today's local policy — no new outbound dependency by default).
...
- 🟡 **Env-blocked, not fake-passed:** the live 12-step E2E + Cloud Run deploy gate that would prove *measured learning impact in production* (integration-graph-proven at the live LangGraph level — strictly stronger than the prior pure-core proof, weaker than production), the live-launch of `executeApprovedSpec` in prod (the API route + service exist, REVIEWER-gated, but the live LangGraph+LLM E2E is env-blocked), the `AuditLog` chain's live-Postgres round-trip + its fetch-latest-then-write TOCTOU under concurrency (best-effort chaining shipped; a sequential per-tenant counter under a lock is the durable fix), the external JAK Shield MCP network transport, and persisted benchmark evaluation (`BENCHMARKS_PERSISTED=false`).
