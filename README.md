...
| Safety / security layer | JAK Shield-style local policy controls, RBAC, approval gates, audit logging (with **per-tenant HMAC row-chain tamper-evidence** — `EVIDENCE_SIGNING_SECRET`-gated, fail-open-to-auditable when unset, and **TOCTOU-safe under concurrency** via a per-tenant `pg_advisory_xact_lock` + partial-unique-seq backstop), PII redaction, a **`ShieldMcpClient`** that signs/verifies high-risk Ed25519 decisions (local embedded mode — now **live-instantiated behind `SHIELD_MCP_CANARY=1`**, observational canary that records signed decisions into the atomic audit chain, does not gate execution), and an **Agent Governance Overlay** (autonomy policy L0-L5 + human-only NEVER set, governance gate). The external JAK Shield MCP *transport* wiring remains a roadmap item; today the enforced gateway is the embedded local one | ✅ Verified |
...
| Agent Optimizer (GEPA) | Google ADK `adk eval` + `GEPARootAgentPromptOptimizer` (20 iters, 102 calls) run against `jak-swarm-gateway`; corrected eval 6/6 training + 4/4 held-out validation (the initial 4/6 was a broken `/api/` path, not agent quality); GEPA Candidate 1 (safety refusal + search_knowledge fallback) adopted in the deployed Agent Engine; results in [`benchmark-optimization-before-after.md`](qa/benchmark-optimization-before-after.md) + [`adk-eval-results.json`](qa/_generated/adk-eval-results.json); latency 4/4 pass, p50 7.6s ([`benchmark-results-gemini.md`](qa/benchmark-results-gemini.md)) | ✅ Verified |
...
6. **Benchmark/readiness scripts and the blocking test suite provide regression protection** — 2,900+ blocking CI tests (unit + integration, 3,157 at last count — 2,734 unit + 423 integration, both blocking in CI) with CI-enforced truth checks (`pnpm check:truth`). Tool maturity labels are CI-enforced. Landing page claims are CI-enforced.

ADK Agent Optimizer (`adk optimize` + `GEPARootAgentPromptOptimizer`, 20 iters / 102 calls) ran against the JAK gateway agent. The baseline prompt was already optimal on the training set (6/6 rubric pass; the initial 4/6 was a broken `/api/` path, not agent quality); GEPA Candidate 1 (explicit safety refusal + `search_knowledge` fallback) matched baseline quality and was adopted in the deployed Agent Engine. A separate held-out validation set (4/4 pass) was added after the original run's train/val overlap was caught. Full results in [`qa/benchmark-optimization-before-after.md`](qa/benchmark-optimization-before-after.md).
...
> **Bounded retry + genuine re-plan loop.** If the Verifier rejects output, the system re-runs the same task against the same worker up to a bounded retry budget (`RepairService` classifies the failure and refuses to auto-retry destructive / permission / approval-timeout / unknown classes; `MAX_TASK_RETRIES=2` is shared by the verifier + edge). Beyond retry, the **HyperAgent self-healing + self-learning layer** (merged to `main`, OFF-gated by `hyperAgentActive(state)` so default workflows are byte-for-byte unchanged unless a tenant opts in) adds a genuine *re-plan* loop: a deterministic 20-class failure classifier → counterfactual fault isolation → a **symbolic replanner** (an LLM may propose, only the symbolic layer may apply) → autonomy-gated selective re-execution. The `executeApprovedSpec` closed loop and the `AuditLog` per-tenant HMAC row-chain are implemented + integration-graph-proven; the `ShieldMcpClient` is **live-instantiated behind `SHIELD_MCP_CANARY=1`** (observational canary — records signed Ed25519 decisions to the atomic audit chain, does not gate execution); the **audit-chain TOCTOU is closed** (per-tenant `pg_advisory_xact_lock` + partial-unique-seq backstop). Destructive actions are never auto-retried.
>
> **Honest scope — what is still env-blocked, not fake-passed:** the live 12-step E2E + Cloud Run deploy gate that would prove *measured learning impact in production* (integration-graph-proven at the live LangGraph level, NOT production-proven); the live-launch of `executeApprovedSpec` in prod (the REVIEWER-gated API route + service exist, but the live LangGraph+LLM E2E is env-blocked); and the external JAK Shield MCP network *transport* (the local embedded Shield is live; the remote-transport seam is roadmap). Full wired-vs-pure-core breakdown in the [Long-Term Vision](#-long-term-vision) section + [`docs/hyperagent-current-state-audit.md`](docs/hyperagent-current-state-audit.md).
...
- ✅ **Closed loops now implemented + integration-graph-proven (NOT production-proven):** the full `executeApprovedSpec` closed loop (`POST /company/specs/:id/execute`, REVIEWER-gated → materialise → run → harvest → acceptance verdict → resolved drift), learning dedup / merge / promote / scope (upsert-by-natural-key `@@unique([tenantId, key])` + MI-truth conflict policy + scope-baked key), and `AuditLog` HMAC per-tenant row-chain hashing (migration 117; tamper-evident; fail-open-to-auditable when `EVIDENCE_SIGNING_SECRET` is unset — the row is still written, just not tamper-evident).
- ✅ **Live-instantiated (PR E), observational canary:** the `ShieldMcpClient` signs/verifies high-risk Ed25519 decisions and is now wired into `swarm-execution.service.ts` behind `SHIELD_MCP_CANARY=1` (default-off). It RECORDS signed decisions into the atomic audit chain but does NOT gate execution — wiring a Shield BLOCK into the execution gate is a deliberate later step. The external JAK Shield MCP *transport* (remote `JAK_SHIELD_MCP_URL`) remains roadmap; when unset, behavior is identical to today's local policy — no new outbound dependency by default.
...
- 🟡 **Env-blocked, not fake-passed:** the live 12-step E2E + Cloud Run deploy gate that would prove *measured learning impact in production* (integration-graph-proven at the live LangGraph level — strictly stronger than the prior pure-core proof, weaker than production), the live-launch of `executeApprovedSpec` in prod (the API route + service exist, REVIEWER-gated, but the live LangGraph+LLM E2E is env-blocked), the external JAK Shield MCP network *transport* (the local embedded Shield is live; the remote-transport seam is roadmap), and persisted benchmark evaluation (`BENCHMARKS_PERSISTED=false`). The `AuditLog` chain's fetch-latest-then-write TOCTOU under concurrency is **CLOSED (PR E)** — the atomic `appendChainedAuditRow` primitive serialises per-tenant appends with `pg_advisory_xact_lock` + a partial-unique-seq backstop (migration 122), proven by concurrent-appender integration tests.

The pure-core + live-wired layer is proven by 3,157 unit + integration tests (2,734 unit + 423 integration); integration tests compose the real graphs and pure cores end-to-end (real plan repair, measured learning promotion at the live-graph level, Shield fail-closed, config rollback, AuditLog chain verify + tamper detection) but do **not** imply those cores run inside every live workflow in production. JAK Swarm stays the execution engine; the HyperAgent layer only observes/proposes/replans; JAK Shield stays the independent trust gateway.
...
**Enterprise SLA packaging, expanded observability, and production hardening are part of the post-build roadmap.** Specific items on that roadmap: live-hosted smoke tests, third-party security audit/certification, lawyer-reviewed ToS/DPA, pen test, a live production canary of the HyperAgent learning loop + Brain/Shield MCP canaries, and an incident-response runbook. (The `AuditLog` per-tenant HMAC row-chain — including TOCTOU-safe concurrent appends — is already shipped.)
