...
- **persistLearning / recallLearnings**: Agents store and retrieve facts across workflows via per-role memory keyed by tenant. The HyperAgent self-learning layer now wires these into the live LangGraph path (OFF-gated): the learning node persists outcome-derived candidates and the planner recalls promoted learnings scoped by task type / industry.
...
| **Company Memory** | Transcripts, decisions, policies, people, projects, risks, evidence as persistent, queryable context | `company-operating-layer.service.ts` implements artifact → entity → drift finding → spec pipeline. `persistLearning` / `recallLearnings` are wired into the live LangGraph path (OFF-gated HyperAgent self-learning node + planner recall), with the live production E2E still env-blocked. Memory is session-scoped; cross-session grounding is evolving. |
| **Role-Based Intelligence** | CEO, HR, CTO, CMO, Finance, Legal, Ops, Support agents with department-scoped context, Ability Packs, Autonomy Ladder (L0–L5), and approval gates | Agent roles exist. Department-scoped RBAC (5 roles) is tenant-scoped, not department-scoped. The **Autonomy Ladder (L0-L5 + a fixed human-only NEVER set)** is built as a central deterministic policy (`packages/security` governance, HyperAgent Phase 1). Department-scoped Ability Packs remain a roadmap item. |
| **Permission + Governance** | Agent Governance Overlay enforcing agent profiles, memory scopes, and autonomy boundaries; JAK Shield MCP for signed security decisions on high-risk actions | Local policy logic in `packages/security` is shipped. The **Agent Governance Overlay** (autonomy policy + governance gate, HyperAgent Phase 7) and the **`ShieldMcpClient`** local signed-decision path (Phase 8) are built. JAK Shield MCP is a separate gateway; the **external MCP transport** wiring remains a roadmap item. Agent Profile Registry is a roadmap item. |
| **Autonomous Execution** | Plan, assign, execute, verify, report, learn again in a self-improving closed loop | Commander → Planner → Router → Worker → Verifier loop is shipped. The **HyperAgent self-healing + self-learning layer** (merged to `main`, Phases 0-15) adds deterministic failure classification, counterfactual diagnosis, symbolic re-plan + validation, an information-theoretic learning gate, versioned-config lifecycle, and R5 code self-repair. **Wired into the live LangGraph graph (OFF-gated, non-regressing):** BOTH the repair half (diagnosis → symbolic replanner → selective re-execution) AND the self-learning half (outcome evaluator + acceptance checker, learning extraction/persist/recall, mutual-information gate, meta-optimiser/bandit selection, R2 `CORRECT_OUTPUT` typed correction) — `hyperAgentEnabled` defaults false so default workflows are byte-for-byte unchanged unless a tenant opts in. **Now implemented + integration-graph-proven (NOT production-proven):** the full `executeApprovedSpec` closed loop (REVIEWER-gated API route → materialise → run → harvest → acceptance verdict → resolved drift), learning dedup/merge/promote/scope, and `AuditLog` HMAC per-tenant row-chain hashing (migration 117; fail-open-to-auditable when `EVIDENCE_SIGNING_SECRET` is unset). **Deferred (pure core, not yet instantiated in the live action path):** the `ShieldMcpClient` (local-embedded + external MCP transport). **Resolved:** retry accounting is unified on `taskRetryCount` + a single shared `MAX_TASK_RETRIES=2` (verifier + edge read the same field/constant), and a 16-bug hardening sweep landed further safety fixes (host-JS `code_execute` production-disabled, clearable graph `error` channel, Shield subject-substitution defense, DP-noise scale fix, cyclic-spec rejection, code-repair path normalization, node-timeout timer-leak fix, risk-classifier security floor unbypassable). **Env-blocked, not fake-passed:** live *measured learning impact in production* (12-step E2E + Cloud Run deploy gate — integration-graph-proven at the live LangGraph level, not production-proven), live-launch of `executeApprovedSpec` in prod, `AuditLog` chain live-Postgres round-trip + its fetch-latest-then-write TOCTOU under concurrency, and the external JAK Shield MCP transport. |
