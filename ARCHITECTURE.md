...
| **Company Memory** | Transcripts, decisions, policies, people, projects, risks, evidence as persistent, queryable context | `company-operating-layer.service.ts` implements artifact → entity → drift finding → spec pipeline. `persistLearning` / `recallLearnings` are defined on `BaseAgent` but are **not yet called from any live execution path** (pure-core only). Memory is session-scoped; cross-session grounding is evolving. |
...
| **Autonomous Execution** | Plan, assign, execute, verify, report, learn again in a self-improving closed loop | Commander → Planner → Router → Worker → Verifier loop is shipped. The **HyperAgent self-healing + self-learning layer** (merged to `main`, Phases 0-15) adds deterministic failure classification, counterfactual diagnosis, symbolic re-plan + validation, an information-theoretic learning gate, versioned-config lifecycle, and R5 code self-repair. **Wired into the live graph:** the repair half (diagnosis → symbolic replanner → selective re-execution, OFF-gated). **Pure-core-tested, not yet wired into the live path:** outcome evaluator, learning extraction/recall, mutual-information gate, meta-optimiser, R2 correction, and the `ShieldMcpClient`. **Not yet implemented:** full `executeApprovedSpec` loop, learning dedup/conflict, `AuditLog` HMAC. **Resolved:** retry accounting is unified on `taskRetryCount` + a single shared `MAX_TASK_RETRIES=2` (verifier + edge read the same field/constant), and a 16-bug hardening sweep landed further safety fixes (host-JS `code_execute` production-disabled, clearable graph `error` channel, Shield subject-substitution defense, DP-noise scale fix, cyclic-spec rejection, code-repair path normalization, node-timeout timer-leak fix, risk-classifier security floor unbypassable). Live *measured learning impact in production* (12-step E2E + Cloud Run deploy gate) is env-blocked and not fake-passed. |
