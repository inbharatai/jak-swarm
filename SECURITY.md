...
- **Agent Governance Overlay** — the autonomy-policy core (L0-L5 capability matrix + a fixed human-only NEVER set: merge/deploy/secrets/payments/prod-data-delete/external-publish/permission-expansion/governance-rewrite) and the governance gate are **built** (HyperAgent Phases 1 + 7, `packages/security`). The `ShieldMcpClient` signs + verifies high-risk decisions in local embedded mode and fail-closes when the Shield is unavailable or its signature is unverified (Phase 8) — this is **unit-tested but not yet instantiated in the live action path** (no high-risk call currently routes through it at runtime). `AuditLog` row-chain HMAC is also not yet implemented. What remains is wiring the client into the live path, the **external JAK Shield MCP transport** call, and full agent profiles / memory scopes / department boundaries.
