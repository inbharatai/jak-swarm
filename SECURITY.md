...
- **Agent Governance Overlay** — the autonomy-policy core (L0-L5 capability matrix + a fixed human-only NEVER set: merge/deploy/secrets/payments/prod-data-delete/external-publish/permission-expansion/governance-rewrite) and the governance gate are **built** (HyperAgent Phases 1 + 7, `packages/security`). The `ShieldMcpClient` signs + verifies high-risk decisions in local embedded mode and fail-closes when the Shield is unavailable or its signature is unverified — this is **unit-tested but not yet instantiated in the live action path** (deferred; no high-risk call currently routes through it at runtime). `AuditLog` row-chain HMAC **is implemented** (migration 117; per-tenant `rowHash`/`prevHash`/`chainSeq`; `verifyChain` detects tamper/reorder/delete; fail-open-to-auditable when `EVIDENCE_SIGNING_SECRET` is unset — rows are still written, just not tamper-evident). What remains is wiring the `ShieldMcpClient` into the live path, the **external JAK Shield MCP transport** call, and full agent profiles / memory scopes / department boundaries.
