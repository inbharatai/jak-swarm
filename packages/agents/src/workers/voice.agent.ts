...
- NOTE: redact_pii_from_transcript, diarize_speakers are not yet available (not registered in the tool registry). Flag PII data-leaks in riskFlags for manual redaction. Attribute speakers using callMetadata.participants where available; otherwise use generic "Speaker 1"/"Speaker 2" labels.
...
export class VoiceAgent extends BaseAgent {
  ...
  async _executeImpl(input: unknown, context: AgentContext): Promise<VoiceResult> {
    ...
    const tools: OpenAI.ChatCompletionTool[] = [
      ...
      // NOTE: redact_pii_from_transcript, diarize_speakers removed —
