 ...
 *
 * Location: this file used to live at `apps/api/src/services/repair.service.ts`
 * but the worker-node (in `packages/swarm/src/graph/nodes/`) needs to
 * call it on failure, and apps/api → packages/swarm is a one-way
 * dependency. P1-3 of the launch-readiness audit moved it into the
 * swarm package; the apps/api file now re-exports for backward compat
 * with existing tests.
 */
...
/**
 * Module-level singleton instance for callers that don't inject their
 * own. The worker-node uses this so the repair flow doesn't require
 * threading a service through SwarmState (which can't carry Function
 * values across DB checkpoint serialization).
 */
export const defaultRepairService = new RepairService();
